...
import { scrubCodenames } from './redactions-core.mjs';
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--margin=0.6in]');
    process.exit(1);
  }
  ...
  return renderHtmlToPdf(html, outputPath, { format, margin, baseDir: dirname(inputPath) });
}
 ...
 * @param {{format?: 'a4'|'letter', margin?: string, baseDir?: string}} [opts]
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  // CIIAA last-resort guard: strip any confidential codename before it can land
  // in a PDF. This shared path renders both CV PDFs and cover-letter PDFs, so a
  // single scrub here covers both. See redactions.yml + redactions-core.mjs.
  html = scrubCodenames(html, { label: relative(process.cwd(), outputPath) || outputPath });
