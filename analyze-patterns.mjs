...
function extractBlockerType(gap) {
  ...
  if (/\b(maya|houdini|blender|3ds ?max|zbrush|substance|nuke|katana|mari|marvelous|unreal|unity|godot|mel|pymel|usd|openusd|materialx|hlsl|glsl|shader|rigging|skinning|groom|look[- ]?dev|metahuman|nanite|lumen|pytorch|diffusion|mlops)\b/.test(desc)) return 'stack-mismatch';
...
function analyze() {
  ...
  for (const e of enriched) {
    ...
    for (const gap of e.report.gaps) {
      ...
      const techs = gap.description.match(/\b(JavaScript|TypeScript|Python|Ruby|Java|Go|Rust|Node\.?js|React|Angular|Vue\.?js|Django|Flask|Rails|PHP|Laravel|Symfony|Kotlin|Swift|C\+\+|C#|\.NET|MongoDB|MySQL|PostgreSQL|Redis|GraphQL|REST|AWS|GCP|Azure|Docker|Kubernetes|Terraform|Supabase|Inngest|React Native|Maya|Houdini|Blender|3ds ?Max|ZBrush|Substance|Nuke|Katana|Mari|Marvelous|Unreal|Unity|Godot|MEL|PyMEL|USD|OpenUSD|MaterialX|HLSL|GLSL|VEX|MetaHuman|Nanite|Lumen|PyTorch|Perforce|rigging|skinning|groom|look-?dev|shader)\b/gi);
      if (techs) {
