...
- All Personal Identifiable Information (PII) you input — CVs, contact details, career history — is stored locally on your machine. It leaves the machine whenever a tool sends it to a model provider: your AI CLI does so on every evaluation, and `gemini-eval.mjs` sends your CV and profile to the Gemini API.
