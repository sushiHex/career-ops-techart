...
function dataRows(trackerPath) {
  return readFileSync(trackerPath, 'utf-8')
    .split('\n')
    // Separator rows only. A bare includes('---') also dropped data rows whose
    // report link contains "---" from a slugified role — see merge-tracker.mjs.
    .filter(l => l.startsWith('|') && !/^\s*\|[\s:|-]*\|\s*$/.test(l) && !/\bScore\b/.test(l));
}
...
// Test: a data row whose report link contains "---" must stay VISIBLE to merge.
// Slugifying a role like "Engineer 5 - Model Runtime" yields
// "engineer-5---model-runtime", and a bare includes('---') separator test then
// skipped the entire row. Two live rows were invisible this way, so a
// re-evaluation of either would have been appended as a DUPLICATE instead of
// updating the existing entry — the one thing this script exists to prevent.
{
  const TRICKY = [
    ...
    '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |',
    '|---|------|---------|------|-------|--------|-----|--------|-------|',
    '| 1 | 2026-01-01 | Acme | Engineer 5 - Model Runtime | 4.0/5 | Applied | x | [1](../reports/1-acme-engineer-5---model-runtime-2026-01-01.md) | seed |',
    '',
  ].join('\n');
  const DUP = ['1', '2026-03-03', 'Acme', 'Engineer 5 - Model Runtime', 'Evaluated',
    '4.2/5', 'x', '[1](reports/1-acme-engineer-5---model-runtime-2026-01-01.md)', 're-eval'].join('\t') + '\n';
  const sb = makeSandbox(TRICKY, { '1-acme.tsv': DUP });
  const merge = runScript('merge-tracker.mjs', [], sb);
  const rows = dataRows(sb.tracker);
  const acmeRows = rows.filter((r) => r.includes('Acme'));
  if (merge.code === 0 && acmeRows.length === 1) {
    pass('row with "---" in its report link stays visible to merge (no duplicate)');
  } else {
    fail(`row with "---" yielded ${acmeRows.length} Acme rows (code ${merge.code})\n${rows.join('\n')}`);
  }
  rmSync(sb.dir, { recursive: true, force: true });
}
