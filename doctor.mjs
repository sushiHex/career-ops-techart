...
const USER_LAYER_PREREQS = [
  ...
  },
  {
    ...
    fix: [
      'Run: node setup.mjs (writes this and the other config files from presets)',
      'Or: cp config/profile.example.yml config/profile.yml, then edit it',
    ],
  },
  {
    ...
    fix: [
      'Run: node setup.mjs',
      'Or: cp modes/_profile.template.md modes/_profile.md, then customize it',
    ],
  },
  {
    path: 'portals.yml',
    fix: [
      'Run: node setup.mjs',
      'Or: cp templates/portals.example.yml portals.yml, then add your companies',
    ],
  },
];
