...
import { buildSignals, decideLocations, effectiveLocationFilter } from './location-core.mjs';
...
/**
 * Compile one title keyword into a matcher.
 *
 * Matching is word-bounded, not substring. Substring matching silently ate real
 * roles: the negative "Intern" rejected "Internal Tools Engineer", and "Sales"
 * rejected anything with "Salesforce" in the title.
 *
 * A trailing `*` opts into stem matching for genuine prefixes ("Advertis*"
 * covers Advertising and Advertisement; "Agent*" covers Agent, Agents and
 * Agentic). Without it, the keyword must match a whole word.
 *
 * @param {string} keyword
 * @returns {(title: string) => boolean}
 */
function compileTitleKeyword(keyword) {
  const isStem = keyword.endsWith('*');
  const body = (isStem ? keyword.slice(0, -1) : keyword).toLowerCase().trim();
  if (!body) return () => false;
  const escaped = body.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
  const tail = isStem ? '' : '(?=$|[^a-z0-9])';
  return (title) => new RegExp(`(^|[^a-z0-9])${escaped}${tail}`, 'i').test(title);
}

export function buildTitleFilter(titleFilter) {
  const positive = (titleFilter?.positive || []).map(compileTitleKeyword);
  const negative = (titleFilter?.negative || []).map(compileTitleKeyword);

  return (title) => {
    const text = String(title ?? '');
    const hasPositive = positive.length === 0 || positive.some(match => match(text));
    const hasNegative = negative.some(match => match(text));
    return hasPositive && !hasNegative;
  };
}

/**
 * Explain a title decision — which keyword decided it, and how.
 *
 * Used by the reject log and _titleaudit.mjs. A filter that drops 97% of its
 * input needs to be able to say why about any single row.
 *
 * @param {object} titleFilter - The portals.yml `title_filter` block.
 * @returns {(title: string) => {pass: boolean, reason: string}}
 */
export function buildTitleExplainer(titleFilter) {
  const positive = (titleFilter?.positive || []).map(k => [k, compileTitleKeyword(k)]);
  const negative = (titleFilter?.negative || []).map(k => [k, compileTitleKeyword(k)]);
  return (title) => {
    const text = String(title ?? '');
    const neg = negative.find(([, match]) => match(text));
    if (neg) return { pass: false, reason: `negative "${neg[0]}"` };
    if (positive.length === 0) return { pass: true, reason: 'no positive list configured' };
    const pos = positive.find(([, match]) => match(text));
    if (pos) return { pass: true, reason: `positive "${pos[0]}"` };
    return { pass: false, reason: 'no positive keyword matched' };
  };
}

// ── Location filter ─────────────────────────────────────────────────
// The scan's location decision comes from config/location.json (see location-core.mjs
// effectiveLocationFilter); portals.yml `location_filter` may only override its lists.
// buildLocationFilter below is the older public helper and keeps its generic allow/block
// contract for callers that hand it a block directly.
...
export function buildLocationFilter(locationFilter) {
  ...
  const signalKeys = [
    'remote_signals', 'us_wide_signals', 'us_state_signals', 'home_state_signals',
    'commute_signals', 'california_signals', 'socal_signals', 'blocked_geo', 'hard_out_metro',
  ];
  // Preserve the public helper's historical generic allow/block behavior for
  // third-party configs; portals.yml opts into the signal policy below.
  if (!signalKeys.some(key => Object.hasOwn(locationFilter, key))) {
    const scoped = normalizeKeywordList(locationFilter.always_allow_scoped);
    const always = normalizeKeywordList(locationFilter.always_allow);
    ...
    return (location) => {
      ...
      if (scoped.some(k => lower.includes(k))) return true;
  ...
  // Signal-policy path delegates to location-core so the scanner, the auditors
  // (_locaudit.mjs, _claimprobe.mjs) and the test suite share one implementation.
  // The boolean contract is preserved for callers that only want a predicate:
  // only a definite `reject` filters a job out. `unknown` passes, because a
  // false accept costs one line of triage while a false reject can hide a job
  // permanently — but scan() records those separately so they stay auditable.
  const signals = buildSignals(locationFilter);
  return (location, meta) => decideLocations(location, signals, meta).verdict !== 'reject';
}

/**
 * Build the full location decision function (accept / reject / unknown).
 *
 * Prefer this over buildLocationFilter inside the scanner: the three-way
 * verdict is what makes an unresolved multi-location posting distinguishable
 * from a genuinely workable one.
 *
 * @param {object} locationFilter - The portals.yml `location_filter` block.
 * @returns {(input: string|string[], meta?: object) => {verdict: string, reason: string, options: string[]}}
 */
export function buildLocationDecider(locationFilter) {
  if (!locationFilter) return () => ({ verdict: 'accept', reason: 'no filter configured', options: [] });
  const signals = buildSignals(locationFilter);
  return (input, meta) => decideLocations(input, signals, meta);
}
...
/**
 * Canonical company name for tracker matching.
 *
 * portals.yml decorates entries for human readability ("Crunchyroll (Sony) —
 * Applied AI (LA hybrid)", "Anduril Industries — Costa Mesa (defense-AI /
 * real-time 3D)") while data/applications.md stores the plain name. An exact
 * key therefore almost never matched for decorated entries, and roles that had
 * already been evaluated came back as new on every scan — 23 of 362 rows on the
 * 2026-08-11 scan.
 */
export function canonicalCompany(value) {
  return String(value ?? '')
    .toLowerCase()
    .split(/\s+[—–]\s+/)[0]          // drop the descriptive tail
    .replace(/\([^)]*\)/g, ' ')       // drop parentheticals
    .replace(/\[[^\]]*\]/g, ' ')      // drop [REMOTE-US]-style tags
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
}

/**
 * Canonical role title for tracker matching.
 *
 * Normalises punctuation and conjunctions ("&" vs "and") and strips trailing
 * requisition ids, which the tracker records and job boards usually do not.
 *
 * CAUTION: stripping the req id means two DISTINCT reqs that share a title
 * collapse to one key — NVIDIA JR2462112 and JR2525326 are both "Senior
 * Engineer - AI Agents and Systems". That is why every match is written to
 * data/already-evaluated.tsv rather than dropped silently: an over-merge stays
 * visible and recoverable instead of quietly deleting a live role.
 */
export function canonicalRole(value) {
  return String(value ?? '')
    .toLowerCase()
    .replace(/\((?:jr|req|r|gh)[- ]?[\w-]+\)/gi, ' ')
    .replace(/\breq[- ]?\d+\b/gi, ' ')
    .replace(/\bjr\d{6,}\b/gi, ' ')
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
}
...
function loadSeenCompanyRoles() {
  ...
  if (existsSync(APPLICATIONS_PATH)) {
    ...
    for (const line of text.split('\n')) {
      ...
      const [, num, , company, role, score, status] = cells;
      if (!/^\d+$/.test(num || '') || !company || !role) continue;
      const key = `${canonicalCompany(company)}::${canonicalRole(role)}`;
      if (!seen.has(key)) seen.set(key, { num, company, role, score, status });
    }
  }
  return seen;
}
...
export function formatScanHistoryRow(offer, date, status = 'added') {
  return [
    ...
    offer.company,
    status,
    // Record the RESOLVED option list when the provider produced one. The raw
    // `location` for a multi-site Workday posting is the placeholder "5
    // Locations", so recording it left history saying something the filter never
    // saw — the decision used the enriched list. That also broke _locaudit.mjs,
    // which replays this file: every enriched row replayed as an unresolved
    // placeholder and reported `unknown`, hiding what the policy actually did.
    (Array.isArray(offer.locations) && offer.locations.length
...
async function main() {
  ...
  const explainTitle = buildTitleExplainer(config.title_filter);
  const locationDecider = buildLocationDecider(effectiveLocationFilter(config.location_filter));
  ...
  function resolveEntries(entries, { isBoard = false } = {}) {
    for (const entry of entries) {
      ...
      // An explicit `scan_method: websearch` is an instruction, not a fallback, so it
      // must be honoured BEFORE provider detection. Several entries carry an ATS-shaped
      // careers_url (Workday, Ashby, SmartRecruiters, Jobvite) whose endpoint is dead or
      // access-restricted, which is precisely why they were set to websearch. Resolving a
      // provider first silently overrode that and scanned them the broken way: Cloud
      // Imperium returned HTTP 422 and Whatnot 404 on every run.
  ...
  // Every location rejection is recorded. Rejected jobs never reach
  // data/scan-history.tsv, so without this list the reject set is invisible and
  // a false reject cannot be discovered by replaying history — the audit is
  // one-directional otherwise.
  const locationRejects = [];
  // Titles vetoed by a NEGATIVE keyword — the class that can silently delete a
  // role the candidate actually wants. See the logging comment at the filter call.
  const titleVetoes = [];
  // Offers skipped because the company+role is already in the tracker. Logged
  // rather than silently dropped: the canonical key strips req ids, so two
  // distinct reqs sharing a title can collapse here, and that must stay visible.
    ...
    try {
      ...
      for (const job of jobs) {
        if (!titleFilter(job.title)) {
          totalFilteredTitle++;
          // Only NEGATIVE-keyword kills are logged. "No positive matched" is
          // ~97% of rejections and is mostly genuine off-domain noise, but a
          // negative keyword vetoing a title is the dangerous class: bare
          // "Marketing" silently killed "Senior Technical Marketing Engineer",
          // a req that went on to reach interview, on every scan for months.
          const why = explainTitle(job.title);
          if (why.reason.startsWith('negative')) {
            titleVetoes.push({ company: job.company, title: job.title, reason: why.reason, url: job.url });
          }
          continue;
        }
        // `job.locations` is the structured option list when a provider could
        // resolve one (see providers/workday.mjs); `job.location` is the raw
        // display string otherwise. Only a definite reject drops the job —
        // `unknown` passes through tagged, so an unresolved posting is triaged
        // by a human rather than silently guessed either way.
        const locationDecision = locationDecider(
          job.locations && job.locations.length ? job.locations : job.location,
          { company: job.company },
        );
        if (locationDecision.verdict === 'reject') {
          totalFilteredLocation++;
          locationRejects.push({
            url: job.url,
            company: job.company,
            title: job.title,
            location: Array.isArray(job.locations) && job.locations.length
              ? job.locations.join(' | ')
              : (job.location || ''),
        ...
        if (locationDecision.verdict === 'unknown') totalLocationReview++;
        job.locationVerdict = locationDecision.verdict;
        ...
        const key = `${canonicalCompany(job.company)}::${canonicalRole(job.title)}`;
        const prior = seenCompanyRoles.get(key);
        if (prior) {
          totalDupes++;
          alreadyEvaluated.push({
            company: job.company, title: job.title, url: job.url,
            priorNum: prior.num, priorScore: prior.score, priorStatus: prior.status,
        ...
        seenCompanyRoles.set(key, { num: '(this scan)', company: job.company, role: job.title, score: '', status: '' });
  ...
  if (titleVetoes.length > 0) {
    const byKeyword = new Map();
    for (const v of titleVetoes) byKeyword.set(v.reason, (byKeyword.get(v.reason) || 0) + 1);
    const top = [...byKeyword.entries()].sort((a, b) => b[1] - a[1]).slice(0, 3)
      .map(([reason, n]) => `${reason.replace(/^negative /, '')}×${n}`).join(', ');
    console.log(`  ...of which vetoed:  ${titleVetoes.length} by a negative keyword (${top})`);
    if (!dryRun) {
      try {
        writeFileSync('data/title-vetoes.tsv', [
          ['company', 'title', 'reason', 'url'].join('\t'),
          ...titleVetoes.map(v => [v.company, v.title, v.reason, v.url]
  ...
  if (totalLocationReview > 0) {
    console.log(`Location needs review: ${totalLocationReview} kept (unrecognized or unresolved)`);
  }
  if (locationRejects.length > 0 && dryRun) {
    console.log(`Location rejects:      ${locationRejects.length} (dry run — log not written)`);
  }
  if (locationRejects.length > 0 && !dryRun) {
    // Persisted so the reject set can be replayed and audited. Overwritten each
    // run: it describes THIS scan's decisions, not an accumulating history.
    const rejectPath = 'data/location-rejects.tsv';
    try {
      writeFileSync(rejectPath, [
        ['url', 'company', 'title', 'location', 'reason'].join('\t'),
        ...locationRejects.map(r => [r.url, r.company, r.title, r.location, r.reason]
          .map(v => String(v ?? '').replace(/[\t\r\n]+/g, ' ').trim()).join('\t')),
      ].join('\n') + '\n');
      console.log(`Location rejects log:  ${rejectPath} (${locationRejects.length} rows)`);
    } catch (err) {
      console.log(`Location rejects log:  could not write ${rejectPath} — ${err.message}`);
    }
  }
  // Distinguish "configured and matched nothing" from "not configured at all".
  // Both printed "0 removed", so a filter that was never switched on looked
  // identical to one doing its job — and neither salary_filter nor
  // content_filter is present in portals.yml (verified 2026-08-12).
  const salaryConfigured = Boolean(config.salary_filter
    && (Number(config.salary_filter.min ?? 0) > 0 || Number(config.salary_filter.max ?? 0) > 0));
  const contentConfigured = Boolean(config.content_filter
    && Object.keys(config.content_filter).length > 0);
  console.log(`Filtered by salary:   ${salaryConfigured ? `${totalFilteredSalary} removed` : 'not configured'}`);
  console.log(`Filtered by content:  ${contentConfigured ? `${totalFilteredContent} removed` : 'not configured'}`);
  console.log(`Duplicates:            ${totalDupes} skipped`);
  if (alreadyEvaluated.length > 0) {
    console.log(`  ...already evaluated: ${alreadyEvaluated.length} matched a tracker row`);
    if (!dryRun) {
      try {
        writeFileSync('data/already-evaluated.tsv', [
          ['company', 'title', 'prior_num', 'prior_score', 'prior_status', 'prior_role', 'url'].join('\t'),
          ...alreadyEvaluated.map(a => [a.company, a.title, a.priorNum, a.priorScore, a.priorStatus, a.priorRole, a.url]
            .map(x => String(x ?? '').replace(/[\t\r\n]+/g, ' ').trim()).join('\t')),
        ].join('\n') + '\n');
      } catch { /* logging must never break a scan */ }
    }
