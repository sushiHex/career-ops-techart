...
7. Rewrite Professional Summary by injecting JD keywords + exit narrative bridge from `config/profile.yml` `narrative.exit_story` (e.g. "Senior backend engineer who turned a legacy billing system into a platform three teams build on. Now bringing that to [JD domain]."). If `_profile.md` defines tracks, lead with the one that matches the detected archetype.
...
14. Write HTML to `output/.tmp/cv-{candidate}-{company}.html` (create `output/.tmp/` if missing — Windows-native, no POSIX `/tmp`)
15. Execute: `node generate-pdf.mjs output/.tmp/cv-{candidate}-{company}.html output/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}` (US studios → `letter`; `generate-pdf.mjs` defaults to a4, so pass `--format=letter` explicitly for US/Canada postings)
