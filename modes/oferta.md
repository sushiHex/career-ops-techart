...
Classify the job into one of the archetypes in `modes/_profile.md` (see `_shared.md` → Archetype Detection). If it is a hybrid, indicate the 2 closest ones. If the archetype table defines a **track**, record it: it selects the comp range (see Block D) and the lead framing. This determines:
- Which proof points to prioritize in block B (per `## Your Adaptive Framing` and `## Portfolio Evidence` in `_profile.md`)
...
**Adapted to the archetype:** lead with what `## Your Adaptive Framing` in `modes/_profile.md` says to emphasize for the detected archetype, and apply its `## Portfolio Evidence` rule where one exists.
...
2. **"Sell senior without lying" plan**: specific phrases adapted to the archetype, concrete achievements to highlight, how to position a career transition or a multi-track background as an advantage rather than a liability
3. **"If they downlevel me" plan**: accept if compensation is fair, negotiate 6-month review, clear promotion criteria

## Block D — Comp and Demand

If the archetype table defines tracks, first pick the comp range that matches the detected **track** (see Step 0): the top-level `compensation` block in `config/profile.yml`, or the `alternate_ranges` entry for that track. Compare the offer against *that* range, not another track's.

Use WebSearch for:
- Current salaries for the role, preferring the industry sources listed under `## Comp Sources` in `modes/_profile.md` when there are any; otherwise Levels.fyi, Glassdoor and Blind
- Company's compensation reputation (industries weight base, bonus, equity and relocation differently)
...
**Selected and framed according to the archetype**, per `## Your Adaptive Framing` in `modes/_profile.md`.

Also include:
- 1 recommended case study, or a portfolio walkthrough where `_profile.md` marks the archetype as portfolio-led (which projects to present and in what order)
- Red-flag questions and how to answer them: the ones listed under `## Red-flag Questions` in `modes/_profile.md`, plus any the CV's gaps against this JD invite
- Confidentiality reminder: never name a confidential project codename when telling these stories; describe the work generically.
